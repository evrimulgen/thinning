...
, T2 as (select * from (select * from T1 where DURATION_S > 0)
         pivot (avg (DURATION_S) for (ALG_NAME) in ('SIMP' as SIMP, 'CALC' as CALC, 'UDAF' as UDAF, 'PPTF' as PPTF, 'MODE' as "MODE")))
select TEST_CASE, to_char (ROW_COUNT, '999G999G999G990') as ROW_COUNT
     ...
     , rtrim (to_char (ROW_COUNT / "MODE", '9999990.00')) as MODE_TRPS
from T2
order by 1, 2;
